...
MODEL='fastmri_unet_dualencoder'
...
python models/unet/train_unet.py --batch-size ${BATCH_SIZE} --num-epochs ${NUM_EPOCHS} --device ${DEVICE} --exp-dir ${EXP_DIR} --data-path ${DATA_PATH} --accelerations ${ACC_FACTOR} --sample-rate ${SAMPLE_RATE} --center-fraction ${CENTER_FRACTION} --challenge ${CHALLENGE}
#ACC_FACTOR_4x
